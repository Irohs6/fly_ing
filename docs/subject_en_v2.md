...
> This will have to be demonstrated during the peer review. *(NEW in v1.5)*
...
- Accepted values: any valid single-word string (e.g., `red`, `blue`, `gray`). There is no fixed list of allowed colors. *(clarified in v1.5)*
...
- **The program must be able to handle any number of drones.** *(NEW in v1.5)*
...
- **The `max_drones` capacity is ignored on `start_hub` and `end_hub` zones** — these have no capacity limit. If such metadata is present on those zones, it is ignored and is not a validation error. *(NEW in v1.5)*
...
- Movement format: `D<ID>-<zone>` or `D<ID>-<connection>` (for drones in transit toward restricted zones).
...
- Total path cost
- Quality and usefulness of visual representation *(NEW in v1.5)*

> Secondary metrics are not mandatory to compute automatically, but learners are encouraged to display them in their simulation output or documentation to help peers evaluate performance.

### Performance Benchmarks

Expected performance levels: *(NEW broader categories in v1.5)*

- **Easy** maps: solved in **less than 10 turns**
- **Medium** maps: solved in **10–30 turns**
- **Hard** maps: solved in **less than 60 turns**
- **Challenger** map *(optional)*: aim to beat the reference record of 45 turns
...
> Additional sections may be required depending on the project (e.g., usage examples, feature list, technical choices, etc.). *(clarified in v1.5)*
...
During evaluation, a **brief modification of the project may be requested** to verify actual understanding. *(expanded in v1.5)*

- The modification can be performed in any development environment you choose.
- It should be feasible **within a few minutes** — unless a specific time frame is defined.
- Examples: update a function, modify a display, adjust a data structure.
- The details (scope, target) will be specified in the evaluation guidelines and may vary between evaluations.
